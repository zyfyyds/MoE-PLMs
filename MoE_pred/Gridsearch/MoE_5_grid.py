...
class MLPInteractionLayer(nn.Module):
    def __init__(self, d_model, hidden_dim=None, dropout=0.4):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(5 * d_model, self.hidden_dim),
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(self.hidden_dim, 5 * d_model),
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        x5_flat = x5.squeeze(1)  # 新增x5
        # 五分支拼接
        combined = torch.cat([x1_flat, x2_flat, x3_flat, x4_flat, x5_flat], dim=1)  # [batch, 5*d_model]
        ...
        x4_out = mlp_out[:, 3*self.d_model:4*self.d_model].unsqueeze(1)
        x5_out = mlp_out[:, 4*self.d_model:].unsqueeze(1)  # 新增x5
...
class CombinedMLPMoEModel(nn.Module):
    def __init__(self, input_dims, d_model=64, hidden_dim_mlp=None, num_experts=8, top_k=2, dropout=0.5):
        ...
        self.proj_x4 = nn.Linear(input_dims[3], d_model)
        self.proj_x5 = nn.Linear(input_dims[4], d_model)  # 新增x5投影
        ...
        # 特征融合（五分支拼接：5*d_model）
        self.fusion_fc = nn.Linear(d_model * 5, d_model)
        self.bn = nn.BatchNorm1d(d_model)
        self.dropout = nn.Dropout(dropout)
        self.attention = nn.MultiheadAttention(
            embed_dim=d_model,
            num_heads=4,  # 可调整的注意力头数
            dropout=dropout,
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        x4_proj = self.proj_x4(x4).unsqueeze(1)
        x5_proj = self.proj_x5(x5).unsqueeze(1)  # 新增x5投影
        
        # MLP特征交互
        x1_mlp, x2_mlp, x3_mlp, x4_mlp, x5_mlp = self.mlp_interaction(
            x1_proj, x2_proj, x3_proj, x4_proj, x5_proj
        )
        ...
        x4_moe = self.moe(x4_mlp).squeeze(1)
        x5_moe = self.moe(x5_mlp).squeeze(1)  # 新增x5的MoE处理
        
        # 五分支拼接融合
        fused = torch.cat([x1_moe, x2_moe, x3_moe, x4_moe, x5_moe], dim=1)  # [batch, 5*d_model]
...
class FeatureDataset(Dataset):
    def __init__(self, x1_feats, x2_feats, x3_feats, x4_feats, x5_feats, target):
        ...
        self.x4 = torch.tensor(x4_feats.values, dtype=torch.float32)
        self.x5 = torch.tensor(x5_feats.values, dtype=torch.float32)  # 新增x5特征
    ...
    def __getitem__(self, idx):
        return self.x1[idx], self.x2[idx], self.x3[idx], self.x4[idx], self.x5[idx], self.target[idx]
...
def load_five_feats(feat_paths, meta_path):
    """加载五分支特征，依赖元数据（含ID和target）"""
    ...
    x4_feat = load_single_feat(feat_paths[3], meta_df)
    x5_feat = load_single_feat(feat_paths[4], meta_df)  # 新增x5
    
    # 校验样本数量
    assert len(x1_feat) == len(x2_feat) == len(x3_feat) == len(x4_feat) == len(x5_feat) == len(meta_df), \
        "特征文件与元数据样本数量不匹配（请检查ID对齐）"
    return x1_feat, x2_feat, x3_feat, x4_feat, x5_feat, meta_df['target']

# 6. 特征归一化（五分支）
def scale_five_features(train_x1, train_x2, train_x3, train_x4, train_x5, 
    ...
    scaler_x5 = MinMaxScaler(feature_range=(0, 1))  # 新增x5归一化器
    ...
    train_x5_scaled = scaler_x5.fit_transform(train_x5)  # 新增x5
    ...
    test_x5_scaled = scaler_x5.transform(test_x5)  # 新增x5
    
    return (
        pd.DataFrame(train_x1_scaled), pd.DataFrame(train_x2_scaled), 
        pd.DataFrame(train_x3_scaled), pd.DataFrame(train_x4_scaled), pd.DataFrame(train_x5_scaled),
        pd.DataFrame(test_x1_scaled), pd.DataFrame(test_x2_scaled), 
        pd.DataFrame(test_x3_scaled), pd.DataFrame(test_x4_scaled), pd.DataFrame(test_x5_scaled)
    )
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, epochs):
    ...
    for epoch in range(epochs):
        total_loss = 0.0
        for x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, batch_target in train_loader:
            x1_batch, x2_batch, x3_batch, x4_batch, x5_batch = (
                x1_batch.to(device), x2_batch.to(device), x3_batch.to(device), 
                x4_batch.to(device), x5_batch.to(device)
            )
            batch_target = batch_target.to(device)
            
            outputs = model(x1_batch, x2_batch, x3_batch, x4_batch, x5_batch)
...
def evaluate_loss(model, data_loader, criterion, device):
    ...
    total_loss = 0.0
    with torch.no_grad():
        for x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, batch_target in data_loader:
            x1_batch, x2_batch, x3_batch, x4_batch, x5_batch = (
                x1_batch.to(device), x2_batch.to(device), x3_batch.to(device), 
                x4_batch.to(device), x5_batch.to(device)
            )
...
# 9. 回归主流程（修改为五分支）
def run_regression_with_params(train_x1, train_x2, train_x3, train_x4, train_x5, 
                              train_target, test_x1, test_x2, test_x3, test_x4, test_x5, 
                              test_target, params):
    ...
    train_x1, val_x1, train_x2, val_x2, train_x3, val_x3, train_x4, val_x4, train_x5, val_x5, train_target, val_target = train_test_split(
        train_x1, train_x2, train_x3, train_x4, train_x5, train_target, test_size=0.2, random_state=42
    )
    
    # 构建数据集
    train_dataset = FeatureDataset(train_x1, train_x2, train_x3, train_x4, train_x5, train_target)
    val_dataset = FeatureDataset(val_x1, val_x2, val_x3, val_x4, val_x5, val_target)
    test_dataset = FeatureDataset(test_x1, test_x2, test_x3, test_x4, test_x5, test_target)
    ...
    # 初始化五分支模型
    input_dims = [train_x1.shape[1], train_x2.shape[1], train_x3.shape[1], train_x4.shape[1], train_x5.shape[1]]
    ...
    print(f"测试集指标 - R²: {metrics_test['r2']:.3f}, MAE: {metrics_test['mae']:.3f}, "
          ...
          f"pearson:{metrics_test['pearson']:.3f}, kendall_t:{metrics_test['kendall_t']:.3f}")
...
# 网格搜索主函数（修改为五分支）
def grid_search(train_x1, train_x2, train_x3, train_x4, train_x5, train_target,
               test_x1, test_x2, test_x3, test_x4, test_x5, test_target, param_grid):
    # 存储所有结果
    all_results = []
    ...
    for i, values in enumerate(product(*param_values)):
        ...
        metrics = run_regression_with_params(
            train_x1, train_x2, train_x3, train_x4, train_x5, train_target,
            test_x1, test_x2, test_x3, test_x4, test_x5, test_target,
            params
        )
        
        # 保存结果
        result = save_results(*metrics, method="MLP+MoE(5branches)_grid_search", params=params)
...
def run_grid_search_on_files(train_feat_paths, train_meta_path, test_feat_paths, test_meta_path, param_grid):
    ...
    - test_meta_path：测试集元数据（含ID和target）
    """
    method = "MLP+MoE(5branches)"
    
    # 加载训练数据
    print("加载训练数据（使用train_meta对齐ID和target）...")
    train_x1, train_x2, train_x3, train_x4, train_x5, train_target = load_five_feats(train_feat_paths, train_meta_path)
    
    # 加载测试数据
    print("加载测试数据（使用test_meta对齐ID和target）...")
    test_x1, test_x2, test_x3, test_x4, test_x5, test_target = load_five_feats(test_feat_paths, test_meta_path)
    
    # 特征归一化
    print("特征归一化...")
    scaled = scale_five_features(
        train_x1, train_x2, train_x3, train_x4, train_x5,
        test_x1, test_x2, test_x3, test_x4, test_x5
    )
    train_x1_scaled, train_x2_scaled, train_x3_scaled, train_x4_scaled, train_x5_scaled, \
    test_x1_scaled, test_x2_scaled, test_x3_scaled, test_x4_scaled, test_x5_scaled = scaled
    
    # 运行回归
    all_results, best_params, best_spearman = grid_search(
        train_x1_scaled, train_x2_scaled, train_x3_scaled, train_x4_scaled, train_x5_scaled, train_target,
        test_x1_scaled, test_x2_scaled, test_x3_scaled, test_x4_scaled, test_x5_scaled, test_target,
        param_grid
    )
...
# 11. 命令行入口（修改为五分支）
def main():
    parser = argparse.ArgumentParser(description="五分支特征回归模型（MLP+MoE）")
    # 训练集参数
    ...
    parser.add_argument("--train_x4", required=True, help="训练集x4特征文件路径")
    parser.add_argument("--train_x5", required=True, help="训练集x5特征文件路径")  # 新增x5
    ...
    parser.add_argument("--test_x4", required=True, help="测试集x4特征文件路径")
    parser.add_argument("--test_x5", required=True, help="测试集x5特征文件路径")  # 新增x5
    parser.add_argument("--test_meta", required=True, help="测试集元数据路径（含ID和target）")
    # 输出参数
    ...
    all_results, best_params, best_spearman = run_grid_search_on_files(
        train_feat_paths=[args.train_x1, args.train_x2, args.train_x3, args.train_x4, args.train_x5],
        train_meta_path=args.train_meta,
        test_feat_paths=[args.test_x1, args.test_x2, args.test_x3, args.test_x4, args.test_x5],
